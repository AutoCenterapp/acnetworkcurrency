...
const char * const BITCOIN_CONF_FILENAME = "acnetwork.conf";
const char * const BITCOIN_PID_FILENAME = "acnetwork.pid";
...
#else
    const char* pszModule = "acnetwork";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ACNETWORK";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ACNETWORK";
#else
    // Unix
    return pathRet / ".acnetwork";
#endif
#endif
}
